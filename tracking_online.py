...
def tracking(args, device):
    ...
    onreader.start()
    frame_ind = 0
    while not onreader.thread_exit:
        # load data
        onreader.thread_lock.acquire()
        frame, lms, frame_num = onreader.get_data()
        onreader.thread_lock.release()
        if lms[29, 0] == 0:
            cv2.waitKey(100)
            continue
        ...
        if frame_ind == 0:
            start_t = time.time()
        frame_ind += 1
        cv2.imshow('FaceVerse Tracking', drive_img[:, :, ::-1])
        print(f'Speed:{(time.time() - start_t) / frame_ind:.4f}, {frame_ind:4} / {frame_num:4}, {total_loss.item():.4f}')

        # keyboard reaction
        keyc = cv2.waitKey(1) & 0xFF
        if keyc == ord('q'):
            onreader.thread_exit = True
        elif keyc == ord('z'):
            half_length = int(half_length * 1.01)
        elif keyc == ord('x'):
            half_length = int(half_length / 1.01)
        elif keyc == ord('a'):
            crop_center[0] += 1
